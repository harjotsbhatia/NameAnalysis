...
# API Gateway
# ----------------------------------------------------------------------
resource "aws_api_gateway_rest_api" "name_rest_api" {
  depends_on = [aws_cloudformation_stack.name_api_sam_stack]
  name       = var.app_name
...
# ----------------------------------------------------------------------
resource "aws_api_gateway_deployment" "name_rest_api_deployment" {
  depends_on  = [aws_api_gateway_rest_api.name_rest_api]
  rest_api_id = aws_api_gateway_rest_api.name_rest_api.id
  stage_name  = "v1"
  variables = {
...
# ----------------------------------------------------------------------
# API Key // multiple can be created
# ----------------------------------------------------------------------
resource "aws_api_gateway_api_key" "api_key" {
  name        = "${var.app_name}-key"
  description = "API Key for name API"
}
...
resource "aws_api_gateway_usage_plan" "usage_plan" {
  ...
  api_stages {
    api_id = aws_api_gateway_rest_api.name_rest_api.id
    stage  = aws_api_gateway_deployment.name_rest_api_deployment.stage_name
  }
}
